...
const CIRRUS_TEXTURE: Texture3D = preload("res://addons/sky_3d/assets/resources/SNoise3D.tres")
const CUMULUS_TEXTURE: Texture3D = preload("res://addons/sky_3d/assets/resources/CNoise3D.tres")
...
func _build_scene() -> void:
	...
	fog_mesh.layers = 1#StaticLayers.COLLISION_LAYERBIT_TERRAIN
...
@export_range(0.0, 15000.0) var fog_start: float = 0.0 :
	set(value):
		fog_start = value
		if is_scene_built:
...
@export_range(0.0, 15000.0) var fog_end: float = 1000.0 :
	set(value):
		fog_end = value
		if is_scene_built:
...
@export var cirrus_texture: Texture3D = CIRRUS_TEXTURE :
	set(value):
